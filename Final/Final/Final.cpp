...
public:
    ...
    void Play(bool guest) {
        ...
        vector<int> quizCountRandomList;

        for (int i = 0; i < quizCount; i++) {
            quizCountRandomList.push_back(i);
        }

        random_shuffle(quizCountRandomList.begin(), quizCountRandomList.end());
        ...
        if (guest) {
            ...
            for (int i = 0; i < len; i++) {
                cout << "Quiz: " << readyQuizes[quizCountRandomList[i]].getQuiz() << endl;
                vector<int> cavablarShuffle;
                auto cavablar = readyQuizes[quizCountRandomList[i]].getAnswers().size();
                for (int j = 0; j < cavablar; j++) {
                    cavablarShuffle.push_back(j);
                }
                random_shuffle(cavablarShuffle.begin(), cavablarShuffle.end());
                for (int j = 0; j < cavablar; j++) {
                    cout << j + 1 << ") " << readyQuizes[quizCountRandomList[i]].getAnswers()[cavablarShuffle[j]].c_str() << endl;
                }
                int answer;
                cin >> answer;
                if (readyQuizes[quizCountRandomList[i]].isAnswerCorrect(readyQuizes[quizCountRandomList[i]].getAnswers()[cavablarShuffle[answer - 1]].c_str())) {
                    score += 10;
                }
                system("cls");
            }
            ...
            for (int i = 0; i < len; i++) {
                ...
                int answer;
                cin >> answer;
                if (readyQuizes[i].isAnswerCorrect(readyQuizes[quizCountRandomList[i]].getAnswers()[cavablarShuffle[answer - 1]].c_str())) {
                    score += 10;
                }
                system("cls");
            }
