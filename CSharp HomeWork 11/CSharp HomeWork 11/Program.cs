...
class Program
{
    ...
    static void Main()
    {
        ...
        List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Lakisha R. Forrest", DateTime.Parse("December 1, 1973"), "224-830-1181", "LakishaRForrest@armyspy.com", "3121 Quarry Drive Montgomery, AL 36117", 3088),
            new Debtor("Pamela H. Beauchampaa", DateTime.Parse("November 20, 1959"), "771-559-6347", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588)
        };

        //2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag

        // foreach (var n in debtors.FindAll(s => s.Email.Split('@')[1] == "dayrep.com" || s.Email.Split('@')[1] == "rhyta.com"))
        //     Console.WriteLine(n);

        //3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        // foreach (var n in debtors.FindAll(s => DateTime.UtcNow.Year - s.BirthDay.Year > 26 && DateTime.UtcNow.Year - s.BirthDay.Year < 36))
        //     Console.WriteLine(n);

        //4) Borcu 5000 - den cox olmayan borclularic cixartmag

        // foreach (var n in debtors.FindAll(s => s.Debt < 5000))
        //     Console.WriteLine(n);

        //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        // Note: Bele bir data yox idi ozum duzeltdim datani.

        // foreach (var n in debtors.FindAll(s => s.FullName.Length > 18 && s.Phone.Count(s => s == '7') >= 2))
        //     Console.WriteLine(n);

        //7) Qishda anadan olan borclulari cixardmaq

        // foreach (var n in debtors.FindAll(s => s.BirthDay.Month == 12))
        //     Console.WriteLine(n);
        ...
        // foreach (var n in debtors.FindAll(s => s.Debt > debtors.Average(s => s.Debt)).OrderByDescending(s => s.FullName).OrderByDescending(s => s.Debt).ToList())
        //     Console.WriteLine(n);

        //9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        // foreach (var n in debtors.FindAll(s => s.Phone.Count(s => s == '7') == 0))
        //     Console.WriteLine($"{n.FullName}, {DateTime.UtcNow.Year - n.BirthDay.Year}, {n.Debt}");
        ...
        // foreach (var n in debtors.FindAll(s => s.FullName.Distinct().Count() >= 3).OrderBy(s => s.FullName).ToList())
        //     Console.WriteLine($"{n.FullName}, {DateTime.UtcNow.Year - n.BirthDay.Year}, {n.Debt}");

        //13)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

        // foreach (var n in debtors.FindAll(s => s.BirthDay.Year == debtors.GroupBy(x => x.BirthDay.Year).OrderByDescending(group => group.Count()).Select(group => group.Key).First()))
        //     Console.WriteLine(n);

        //14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

        // foreach (var n in debtors.OrderByDescending(x => x.Debt).Take(5).ToList())
        //     Console.WriteLine(n);

        //15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq

        // Console.WriteLine(debtors.Sum(x => x.Debt));

        //16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        //foreach (var n in debtors.FindAll(x => x.BirthDay.Year > 1940 && x.BirthDay.Year < 1946))
        //    Console.WriteLine(n);
        ...
        // foreach (var n in debtors.FindAll(s => ((s.BirthDay.Month - 1) * 500 > s.Debt)))
        //     Console.WriteLine(n);

        //20)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq

        // foreach (var n in debtors.FindAll(s => s.FullName.Count(s => s == 's') != 0 && s.FullName.Count(s => s == 'm') != 0 && s.FullName.Count(s => s == 'i') != 0 && s.FullName.Count(s => s == 'l') != 0 && s.FullName.Count(s => s == 'e') != 0))
        //     Console.WriteLine(n);
    }
}